...
//                            linkNodes = doc.SelectNodes("//image");
                        foreach (XmlNode node in linkNodes)
                        {
                            string sSrc = Util.GetAttributeValue(node.Attributes["src"]);
                            string sc_mediaid = Util.GetAttributeValue(node.Attributes["sc_mediaid"]);
                            if (sc_mediaid == null)
                                sc_mediaid = Util.GetAttributeValue(node.Attributes["mediaid"]);
                            if (sSrc != "")
                            {
                        ...
                        {
                            string sSrc = Util.GetAttributeValue(node.Attributes["src"]);
                            string sMediaid = Util.GetAttributeValue(node.Attributes["mediaid"]);
                            if (sSrc != "")
                            {
                                // This is an existing gallery item, since it has the full path
                                if (sSrc.IndexOf("//") > -1)
